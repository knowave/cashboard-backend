...
class WeeklyReportProviderTest {
	...
	fun `월요일에는 직전 월요일부터 일요일까지 집계하고 전주와 비교한다`() {
		...
		monthlyBudgetRepository.budget = MonthlyBudget("2026-09", 500_000, 120_000)

		val summary = provider.generate(LocalDate.of(2026, 9, 7))
...
private class WeeklyMonthlyBudgetRepository : MonthlyBudgetRepository {
	...
	override fun findByTargetMonth(targetMonth: String) = budget?.takeIf { it.targetMonth == targetMonth }
