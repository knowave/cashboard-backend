...
class ExpenseAnalysisServiceImplTest {
	...
	fun `지출이 없으면 총액은 0이고 카테고리 목록은 비어있다`() {
		given(expenseAnalysisRepository.findCategoryExpenses(userId, LocalDate.of(2026, 8, 1), LocalDate.of(2026, 9, 1)))
			.willReturn(emptyList())
		given(expenseAnalysisRepository.findMonthlyExpenses(userId, LocalDate.of(2026, 3, 1), LocalDate.of(2026, 9, 1)))
			.willReturn(emptyList())

		val result = expenseAnalysisServiceImpl.getAnalysis(userId, 2026, 8)
	...
	fun `연도가 바뀌는 1월 조회 시 monthly 조회 구간이 전년도까지 걸쳐있고 전월 비교도 올바르다`() {
		given(expenseAnalysisRepository.findCategoryExpenses(userId, LocalDate.of(2026, 1, 1), LocalDate.of(2026, 2, 1)))
		...
		given(expenseAnalysisRepository.findMonthlyExpenses(userId, LocalDate.of(2025, 8, 1), LocalDate.of(2026, 2, 1)))
			.willReturn(listOf(FakeMonthlyExpenseProjection("2025-12", 400000)))

		val result = expenseAnalysisServiceImpl.getAnalysis(userId, 2026, 1)

		verify(expenseAnalysisRepository).findMonthlyExpenses(userId, LocalDate.of(2025, 8, 1), LocalDate.of(2026, 2, 1))
	...
	fun `카테고리 ratio는 HALF_UP 정책으로 소수 둘째자리까지 반올림된다`() {
		given(expenseAnalysisRepository.findCategoryExpenses(userId, LocalDate.of(2026, 3, 1), LocalDate.of(2026, 4, 1)))
			.willReturn(
				listOf(
		...
		given(expenseAnalysisRepository.findMonthlyExpenses(userId, LocalDate.of(2025, 10, 1), LocalDate.of(2026, 4, 1)))
			.willReturn(emptyList())

		val result = expenseAnalysisServiceImpl.getAnalysis(userId, 2026, 3)
