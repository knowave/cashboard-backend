...
class AssetGoalNotificationPolicyTest {
	...
	fun `40에서 105퍼센트가 되면 모든 marker와 달성 알림만 반환한다`() {
		val decision = policy.evaluate(goalId, "1억 만들기", 100L, 100L, 40L, 105L, occurredAt)
	...
	fun `50과 80 경계를 각각 통과하면 진행 알림을 반환한다`() {
		val atFifty = policy.evaluate(goalId, "내 집", 100L, 100L, 49L, 50L, occurredAt)
		val atEighty = policy.evaluate(goalId, "내 집", 100L, 100L, 79L, 80L, occurredAt)
	...
	fun `달성률이 하락하면 알림 후보를 반환하지 않는다`() {
		val decision = policy.evaluate(goalId, "내 집", 100L, 100L, 80L, 40L, occurredAt)
	...
	fun `0 이하 목표 금액은 알림 후보를 반환하지 않는다`() {
		val decision = policy.evaluate(goalId, "내 집", 0L, 100L, 0L, 100L, occurredAt)
	...
	fun `큰 금액에서도 교차 곱으로 100퍼센트 경계를 정확히 판별한다`() {
		...
		val decision = policy.evaluate(goalId, "큰 목표", target, target, target - 1, target, occurredAt)
