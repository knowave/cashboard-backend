...
class PaymentDuePolicyTest {
	...
	fun `당일 D1 D3 지출 일정은 결제 예정 알림 후보가 된다`(offset: Int) {
		...
		val candidate = policy.evaluate(baseDate, expenseOccurrence(occurrenceDate), scheduledAt, userId)
	...
	fun `결제일까지 남은 날짜에 따라 알림 문구가 구분된다`() {
		assertThat(policy.evaluate(baseDate, expenseOccurrence(baseDate), scheduledAt, userId)?.message)
			.isEqualTo("오늘 전기요금 12,000원이 결제 예정이에요.")
		assertThat(policy.evaluate(baseDate, expenseOccurrence(baseDate.plusDays(1)), scheduledAt, userId)?.message)
			.isEqualTo("내일 전기요금 12,000원이 결제 예정이에요.")
		assertThat(policy.evaluate(baseDate, expenseOccurrence(baseDate.plusDays(3)), scheduledAt, userId)?.message)
			.isEqualTo("3일 후 전기요금 12,000원이 결제 예정이에요.")
	}

	@Test
	fun `수입 일정과 D2 및 조회 범위 밖 일정은 제외한다`() {
		assertThat(policy.evaluate(baseDate, incomeOccurrence(baseDate.plusDays(1)), scheduledAt, userId)).isNull()
		assertThat(policy.evaluate(baseDate, expenseOccurrence(baseDate.plusDays(2)), scheduledAt, userId)).isNull()
		assertThat(policy.evaluate(baseDate, expenseOccurrence(baseDate.minusDays(1)), scheduledAt, userId)).isNull()
		assertThat(policy.evaluate(baseDate, expenseOccurrence(baseDate.plusDays(4)), scheduledAt, userId)).isNull()
	}
